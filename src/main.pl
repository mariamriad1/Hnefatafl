play :-
    ...
    read(Human_role),
    game_loop(Board, attacker, Human_role).

game_loop(Board, Curr_player, Human_role) :-
    print_board(Board),
    (defenders_win(Board) -> 
        write('Defenders Win!') ;
    attackers_win(Board) -> 
        write('Attackers Win!') ;
    take_turn(Board, Curr_player, Human_role, NewBoard),
    switch_player(Curr_player, Next_player),
    game_loop(NewBoard, Next_player, Human_role)).
