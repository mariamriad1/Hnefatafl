...
difficulty(easy,1). difficulty(medium,3). difficulty(hard,5).
...
get_best_move(Board, Difficulty, BestMove) :-
    ...
    findall(V-M, (
        ...
        alphabeta(NB,Depth,-1,10001,defender,V)
    ), Scored),
    max_member(_-BestMove, Scored).

alphabeta(Board,0,_A,_B,_,Value) :- !,
    utility(Board,defender,S), Value is -S.
alphabeta(Board,_D,_A,_B,_,Value) :-
    (defenders_win(Board) -> Value is -10000 ;
     attackers_win(Board) -> Value is 10000), !.
alphabeta(Board,D,A,B,attacker,Value) :-
    ...
    findall(move(R1,C1,R2,C2), (
        ...
        is_valid_move(Board,R1,C1,R2,C2)), Moves),
    (Moves=[] -> Value=-10000
    ; D1 is D-1, ab_max(Moves,Board,D1,A,B,Value)).
alphabeta(Board,D,A,B,defender,Value) :-
    ...
    findall(move(R1,C1,R2,C2), (
        ...
        is_valid_move(Board,R1,C1,R2,C2)), Moves),
    (Moves=[] -> Value=10000
    ; D1 is D-1, ab_min(Moves,Board,D1,A,B,Value)).
...
ab_max([move(R1,C1,R2,C2)|Rest],Board,D,A,B,Value) :-
    ...
    NA is max(A,V),
    (NA>=B -> Value=NA ; ab_max(Rest,Board,D,NA,B,Value)).
...
ab_min([move(R1,C1,R2,C2)|Rest],Board,D,A,B,Value) :-
    ...
    NB2 is min(B,V),
    (A>=NB2 -> Value=NB2 ; ab_min(Rest,Board,D,A,NB2,Value)).
